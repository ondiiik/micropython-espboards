...
EpdiyHighlevelState epd_hl_init(const EpdWaveform* waveform) {
  ...
  mp_obj_array_t* ba = new_ba(fb_size);
  state.front_fb_mp  = ba;
  state.front_fb     = ba->items;
  
  ba = new_ba(fb_size);
  state.back_fb_mp = ba->items;
  state.back_fb    = ba->items;
  
  ba = new_ba(2 * fb_size);
  state.difference_fb_mp = ba;
  state.difference_fb    = ba->items;
