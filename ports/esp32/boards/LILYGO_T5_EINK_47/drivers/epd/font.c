...
extern int uncompress(uint8_t* dest, unsigned long* destLen, const uint8_t* source, unsigned long sourceLen);
...
*/
/*
static void draw_char(sFONT* font, uint8_t *buffer, int *cursor_x, int cursor_y, uint16_t buf_width,
...
static void draw_char(const GFXfont *font, uint8_t *buffer,
                ...
                const FontProperties *properties)
{
    ...
    if (framebuffer == NULL) {
        buf_width = (w / 2 + w % 2);
        buf_height = h;
        buffer = (uint8_t *)m_malloc(buf_width * buf_height);
