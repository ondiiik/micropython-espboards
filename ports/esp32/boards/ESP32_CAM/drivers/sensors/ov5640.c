...
#ifdef REG_DEBUG_ON
    if (ret < 0) {
        MP_LOGE(TAG, "READ REG 0x%04x FAILED: %d", reg, ret);
    }
#endif
    return ret;
}
...
static int write_reg(uint8_t slv_addr, const uint16_t reg, uint8_t value){
    ...
    if ((uint8_t)old_value != value) {
        MP_LOGI(TAG, "NEW REG 0x%04x: 0x%02x to 0x%02x", reg, (uint8_t)old_value, value);
        ret = SCCB_Write16(slv_addr, reg, value);
    } else {
        MP_LOGD(TAG, "OLD REG 0x%04x: 0x%02x", reg, (uint8_t)old_value);
        ret = SCCB_Write16(slv_addr, reg, value);//maybe not?
    }
    if (ret < 0) {
        MP_LOGE(TAG, "WRITE REG 0x%04x FAILED: %d", reg, ret);
    }
#endif
    return ret;
}
...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pre_div, bool root_2x, int pclk_root_div, bool pclk_manual, int pclk_div)
{
    ...
    MP_LOGD(TAG, "Calculated XVCLK: %d Hz, REFIN: %u Hz, VCO: %u Hz, PLL_CLK: %u Hz, SYSCLK: %u Hz, PCLK: %u Hz", xclk, REFIN, VCO, PLL_CLK, SYSCLK, PCLK);
    return SYSCLK;
}
...
static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x, uint8_t pclk_root_div, bool pclk_manual, uint8_t pclk_div){
    ...
    if(ret){
        MP_LOGE(TAG, "set_sensor_pll FAILED!");
    }
    return ret;
}
...
static int reset(sensor_t *sensor)
{
    ...
    if(ret){
        MP_LOGE(TAG, "Software Reset FAILED!");
        return ret;
    }
    vTaskDelay(100 / portTICK_PERIOD_MS);
    ret = write_regs(sensor->slv_addr, sensor_default_regs);
    if (ret == 0) {
        MP_LOGD(TAG, "Camera defaults loaded");
        vTaskDelay(100 / portTICK_PERIOD_MS);
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    default:
        MP_LOGE(TAG, "Unsupported pixformat: %u", pixformat);
        return -1;
    }
    ...
    if(ret == 0) {
        ...
        MP_LOGD(TAG, "Set pixformat to: %u", pixformat);
    }
    return ret;
}
...
static int set_image_options(sensor_t *sensor)
{
    ...
    if(write_reg(sensor->slv_addr, TIMING_TC_REG20, reg20)
        ...
        MP_LOGE(TAG, "Setting Image Options Failed");
        return -1;
    }
    ...
    MP_LOGD(TAG, "Set Image Options: Compression: %u, Binning: %u, V-Flip: %u, H-Mirror: %u, Reg-4514: 0x%02x",
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if(framesize > FRAMESIZE_QSXGA){
        MP_LOGE(TAG, "Invalid framesize: %u", framesize);
        return -1;
    }
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set framesize to: %ux%u", w, h);
    }
    return ret;

fail:
    sensor->status.framesize = old_framesize;
    MP_LOGE(TAG, "Setting framesize to: %ux%u failed", w, h);
    return ret;
}
...
static int set_hmirror(sensor_t *sensor, int enable)
{
    ...
    ret = set_image_options(sensor);
    if (ret == 0) {
        MP_LOGD(TAG, "Set h-mirror to: %d", enable);
    }
    return ret;
}
...
static int set_vflip(sensor_t *sensor, int enable)
{
    ...
    ret = set_image_options(sensor);
    if (ret == 0) {
        MP_LOGD(TAG, "Set v-flip to: %d", enable);
    }
    return ret;
}
...
static int set_quality(sensor_t *sensor, int qs)
{
    ...
    if (ret == 0) {
        ...
        MP_LOGD(TAG, "Set quality to: %d", qs);
    }
    return ret;
}
...
static int set_colorbar(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        ...
        MP_LOGD(TAG, "Set colorbar to: %d", enable);
    }
    return ret;
}
...
static int set_gain_ctrl(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set gain_ctrl to: %d", enable);
...
static int set_exposure_ctrl(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set exposure_ctrl to: %d", enable);
...
static int set_whitebal(sensor_t *sensor, int enable)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set awb to: %d", enable);
...
static int set_agc_gain(sensor_t *sensor, int gain)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set agc_gain to: %d", gain);
...
static int set_aec_value(sensor_t *sensor, int value)
{
    ...
    if (max_val < 0) {
        MP_LOGE(TAG, "Could not read max aec_value");
        return -1;
    }
    if (value > max_val) {
        value =max_val;
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set aec_value to: %d / %d", value, max_val);
...
static int set_ae_level(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set ae_level to: %d", level);
...
static int set_wb_mode(sensor_t *sensor, int mode)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set wb_mode to: %d", mode);
...
static int set_special_effect(sensor_t *sensor, int effect)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set special_effect to: %d", effect);
...
static int set_saturation(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set saturation to: %d", level);
...
static int set_sharpness(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set sharpness to: %d", level);
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t level)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set gainceiling to: %d", l);
...
static int set_denoise(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        MP_LOGD(TAG, "Set denoise to: %d", level);
